    ...
    <table class="table table-hover col-lg-11 mx-auto table-borderless" id="tableListExam">
        ...
        <tbody>
            ...
            <?php if (count($this->view->listExam) >= 1) {
                ...
                <tr class="mt-4">
                    <td colspan="5" style="background-color: #ECECEC;"> <i class="far fa-sticky-note mr-3"></i> <?= $sum ?> pontos já atribuidos na <?= $this->view->listExam[0]->unity ?> unidade de <?= $this->view->listExam[0]->discipline_name ?> ( Restam <?= 10 - $sum ?> )</td>
                </tr>

            <?php } else { ?>

                <tr class="mt-4">
                    <td colspan="4" style="pointer-events:none">Nenhuma avaliação encrontada</td>
                </tr>
