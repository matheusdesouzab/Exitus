...
class Student extends People
{
    ...
    public function profile()
    {
        ...
        $query = "SELECT aluno.id_aluno AS student_id , aluno.nome_aluno AS student_name , aluno.cpf_aluno AS student_cpf , sexo.id_sexo AS student_sex_id , sexo.sexo AS student_sex , aluno.data_nascimento_aluno AS student_birth_date , aluno.naturalidade_aluno AS student_naturalness , aluno.foto_perfil_aluno AS student_profile_photo , aluno.nacionalidade_aluno AS student_nacionality , aluno.nome_mae AS student_mother , aluno.nome_pai AS student_father , tipo_sanguineo.tipo_sanguineo AS blood_type , tipo_sanguineo.id_tipo_sanguineo AS blood_type_id , pcd.pcd AS student_pcd , pcd.id_pcd AS student_pcd_id , telefone.numero_telefone AS telephone_number , endereco.id_endereco AS student_address_id , endereco.cep AS student_zipCode , endereco.bairro AS student_district , endereco.endereco AS student_address , endereco.uf AS student_uf , endereco.municipio AS student_county , serie.sigla AS acronym_series , cedula_turma.cedula AS ballot , curso.sigla AS course , turno.nome_turno AS shift_name , numero_sala_aula.numero_sala_aula AS number_classroom , situacao_aluno.situacao_aluno as student_situation , endereco.id_endereco as address_id , telefone.id_telefone as telephone_id
        FROM aluno LEFT JOIN sexo ON(aluno.fk_id_sexo_aluno = sexo.id_sexo) LEFT JOIN pcd ON(aluno.fk_id_aluno_pcd = pcd.id_pcd ) LEFT JOIN tipo_sanguineo ON(tipo_sanguineo.id_tipo_sanguineo = aluno.fk_id_tipo_sanguineo_aluno) LEFT JOIN telefone ON(aluno.fk_id_telefone_aluno = telefone.id_telefone) LEFT JOIN endereco ON(aluno.fk_id_endereco_aluno = endereco.id_endereco) LEFT JOIN matricula ON(aluno.id_aluno = matricula.fk_id_aluno) LEFT JOIN situacao_aluno ON(matricula.fk_id_situacao_aluno = situacao_aluno.id_stuacao_aluno) LEFT JOIN turma ON(matricula.fk_id_turma_matricula = turma.id_turma) LEFT JOIN serie ON(turma.fk_id_serie = serie.id_serie) LEFT JOIN curso ON(turma.fk_id_curso = curso.id_curso) LEFT JOIN cedula_turma ON(turma.fk_id_cedula = cedula_turma.id_cedula_turma) LEFT JOIN turno ON(turma.fk_id_turno = turno.id_turno) LEFT JOIN sala ON(turma.fk_id_sala = sala.id_sala) LEFT JOIN numero_sala_aula ON(sala.fk_id_numero_sala = numero_sala_aula.id_numero_sala_aula) WHERE aluno.id_aluno = :id";
